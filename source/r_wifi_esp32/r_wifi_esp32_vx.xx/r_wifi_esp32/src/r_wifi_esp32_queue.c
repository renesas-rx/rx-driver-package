...
void wifi_init_request_queue(void)
{
...
{
	if(g_wifi_at_execute_queue[g_wifi_get_queue_index].at_command_id != ESP32_COMMAND_NONE)
	{
		...
		if(g_wifi_at_execute_queue[i].ticket_no == ticket_no)
		{
			if((g_wifi_at_execute_queue[i].result != WIFI_RETURN_ENUM_PROCESSING))
			{
				*result = g_wifi_at_execute_queue[i].result;
				g_wifi_at_execute_queue[i].at_command_id = ESP32_COMMAND_NONE;
				break;
			}
		}
	}
