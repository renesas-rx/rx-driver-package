 ...
 * g_wifi_semaphore flag at one time.
 */
...
wifi_err_t R_WIFI_ESP32_Open(void)
{
    ...
    g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff = g_wifi_at_command_buff;
    g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff_size = sizeof(g_wifi_at_command_buff);
    ...
    pcbuff = g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff;

	ret = wifi_serial_open(WIFI_UART_BOUDRATE_DEFAULT);
	if(ret != 0)
	{
	...
	ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, NULL, 10000, WIFI_RETURN_ENUM_READY, ESP32_SET_REBOOT, 0xff);
	if(ret != 0)
	{
	...
	ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, "AT+GMR\r\n", 10000, WIFI_RETURN_ENUM_OK, ESP32_GET_MODULE_VERSION, 0xff);
	if(ret != 0)
	{
		return WIFI_ERR_MODULE_COM;
	}

	R_SCI_Control(g_wifi_uart[WIFI_UART_COMMAND_PORT].wifi_uart_sci_handle, SCI_CMD_RX_Q_FLUSH, NULL);

#if (WIFI_CFG_SCI_USE_FLOW_CONTROL == 1)
	sprintf((char *)pcbuff, "AT+UART_CUR=%d,8,1,0,1\r\n",WIFI_CFG_SCI_BOUDRATE);
#else
	sprintf((char *)pcbuff, "AT+UART_CUR=%d,8,1,0,0\r\n",WIFI_CFG_SCI_BOUDRATE);
#endif
	ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, pcbuff, 10000, WIFI_RETURN_ENUM_OK, ESP32_SET_UART_HISPEED, 0xff);
	if(ret != 0)
	{
		return WIFI_ERR_MODULE_COM;
	}
	ret = wifi_serial_close();

	ret = wifi_serial_open(WIFI_CFG_SCI_BOUDRATE);
	if(ret != 0)
	{
		return WIFI_ERR_SERIAL_OPEN;
	}
...
wifi_err_t R_WIFI_ESP32_SetDnsServerAddress(uint32_t dnsaddress1, uint32_t dnsaddress2)
{
	...
	wifi_err_t api_ret = WIFI_SUCCESS;
    uint8_t mutex_flag;
	uint8_t *pcbuff = g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff;
	...
	if(WIFI_SUCCESS == api_ret)
	{
		...
		ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, pcbuff, 10000, WIFI_RETURN_ENUM_OK, ESP32_SET_DNS_SRV_ADDRESS, 0xff);
		if(ret != 0)
		{
...
wifi_err_t R_WIFI_ESP32_Connect (uint8_t *pssid, uint8_t *ppass, uint32_t security, uint8_t dhcp_enable, wifi_ip_configuration_t *pipconfig)
{
	...
	if(WIFI_SUCCESS == api_ret)
	{
		if(0 == dhcp_enable)
		{
			...
			if(WIFI_SUCCESS == api_ret)
			{
				...
				ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, pcbuff, 10000, WIFI_RETURN_ENUM_OK, ESP32_SET_STATIC_IP, 0xff);
				if(ret != 0)
				{
					api_ret = WIFI_ERR_MODULE_COM;
				}
	...
	if(WIFI_SUCCESS == api_ret)
	{
		...
		ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, pcbuff, 30000, WIFI_RETURN_ENUM_OK, ESP32_SET_WIFI_CONNECT, 0xff);
		if(ret != 0)
		{
			api_ret = WIFI_ERR_MODULE_COM;
		}
...
int32_t R_WIFI_ESP32_IsConnected (void)
{
	...
	if(g_wifi_ipconfig.ipaddress != 0)
	{
		ret = 0;
	}
	return ret;
...
wifi_err_t R_WIFI_ESP32_SocketOptRequireTls (int32_t socket_no, uint8_t ssl_type, uint8_t cert_key_id, uint8_t ca_id)
{
	int32_t ret;
	uint8_t mutex_flag;
	wifi_err_t api_ret = WIFI_SUCCESS;
...
wifi_err_t R_WIFI_ESP32_SocketConnect (int32_t socket_no, uint32_t ipaddr, uint16_t port)
{
	...
	if(WIFI_SUCCESS == api_ret)
	{
		...
		{
			sprintf((char *)g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff,"AT+CIPSTART=%d,\"TCP\",\"%d.%d.%d.%d\",%d\r\n",
					socket_no,
			...
			ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff, 10000, WIFI_RETURN_ENUM_OK, ESP32_SET_SOCKET_CONNECT, socket_no);
			if(ret != 0)
			{
				api_ret = WIFI_ERR_MODULE_COM;
			}
		...
		{
			sprintf((char *)g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff,"AT+CIPSSLCCONF=%d,%d,%d,%d\r\n",
					socket_no, g_wifi_socket[socket_no].ssl_type, g_wifi_socket[socket_no].ssl_cert_key_id, g_wifi_socket[socket_no].ssl_ca_id );
			ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff, 10000, WIFI_RETURN_ENUM_OK, ESP32_SET_SSLCONFIG, socket_no);
			if( 0 != ret )
			{
				api_ret = WIFI_ERR_MODULE_COM;
			}
			if(WIFI_SUCCESS == api_ret)
			{
				sprintf((char *)g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff,"AT+CIPSTART=%d,\"SSL\",\"%d.%d.%d.%d\",%d\r\n",
						socket_no, (uint8_t)(ipaddr>>24),(uint8_t)(ipaddr>>16),(uint8_t)(ipaddr>>8),(uint8_t)(ipaddr),port);
				ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff, 20000, WIFI_RETURN_ENUM_OK, ESP32_SET_SOCKET_CONNECT, socket_no);
				if( 0 != ret )
				{
					api_ret = WIFI_ERR_MODULE_COM;
				}
...
int32_t R_WIFI_ESP32_SocketSend (int32_t socket_no, uint8_t *pdata, int32_t length, uint32_t timeout_ms)
{
	...
	if( (socket_no >= WIFI_CFG_CREATABLE_SOCKETS) || (socket_no < 0) || (g_wifi_socket[socket_no].socket_status != WIFI_SOCKET_STATUS_CONNECTED) )
	{
		return  WIFI_ERR_SOCKET_NUM;
	}
    ...
    if(0 == wifi_take_mutex(mutex_flag))
	{
    	if(0 == g_wifi_socket[socket_no].socket_create_flag || (g_wifi_socket[socket_no].socket_status != WIFI_SOCKET_STATUS_CONNECTED))
    	{
    	...
    	else
    	{
			...
			while(sended_length < length)
			{
				...
				sprintf((char *)g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff, "AT+CIPSEND=%d,%d\r\n" ,socket_no, current_send_length);

				ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff, 10000, WIFI_RETURN_ENUM_OK_GO_SEND, ESP32_SET_SOCKET_SEND_START, socket_no);
				...
				ticket_no = wifi_set_request_in_queue( ESP32_SET_SOCKET_SEND_DATA, socket_no );
...
#if DEBUGLOG == 1
		R_BSP_CpuInterruptLevelWrite (14);
		printf("wifi_tcp_disconnect(%d)\r\n",socket_no);
		R_BSP_CpuInterruptLevelWrite (0);
#endif
		sprintf((char *)g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff,"AT+CIPCLOSE=%d\r\n",socket_no);
		subroutain_ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff, 10000, WIFI_RETURN_ENUM_OK, ESP32_SET_SOCKET_CLOSE, socket_no);
		if(subroutain_ret != 0)
		{
	    	api_ret = WIFI_ERR_MODULE_COM;
		}
...
wifi_err_t R_WIFI_ESP32_DnsQuery (uint8_t *pdomain_name, uint32_t *pipaddress)
{
	...
	if(WIFI_SUCCESS == api_ret)
	{
		strcpy((char *)g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff,"AT+CIPDOMAIN=\"");
		sprintf((char *)g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff+strlen((char *)g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff),"%s\"\r\n",pdomain_name);

		func_ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff, 20000, WIFI_RETURN_ENUM_OK, ESP32_SET_DNSQUERY, 0xff);
		if(func_ret != 0)
		{
			api_ret = WIFI_ERR_MODULE_COM;
		}
...
wifi_err_t R_WIFI_ESP32_Ping (uint32_t ipaddr, uint16_t count, uint32_t intervalms)
{
	...
	for(i=0;i<count;i++)
	{
		...
		if(WIFI_SUCCESS == api_ret)
		{
			strcpy((char *)g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff,"AT+PING=\"");
			sprintf((char *)g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff+strlen((char *)g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff),"%d.%d.%d.%d\"\r\n",
					(uint8_t)(ipaddr >> 24),(uint8_t)((ipaddr >> 16) & 0xFF), (uint8_t)((ipaddr >> 8) & 0xFF), (uint8_t)((ipaddr) & 0xFF));
			func_ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, g_wifi_uart[WIFI_UART_COMMAND_PORT].comannd_buff, 20000, WIFI_RETURN_ENUM_OK, ESP32_SET_PING, 0xff);
			if(func_ret != 0)
			{
				api_ret = WIFI_ERR_MODULE_COM;
			}
