 ...
 ***********************************************************************************************************************/
/*********************************************************************************************************************
 * File Name    : r_lpc_rx210.c
 * Description  : Implementation of the LPC module for the RX210
 **********************************************************************************************************************/
/*********************************************************************************************************************
 ...
 *                    LPC_OP_HIGH_SPEED
 *                    LPC_OP_MIDDLE_SPEED_1A
 *                    LPC_OP_MIDDLE_SPEED_1B
 *                    LPC_OP_LOW_SPEED_1
 *                    LPC_OP_LOW_SPEED_2
...
lpc_err_t lpc_low_power_mode_configure (lpc_low_power_mode_t e_mode)
{
    switch (e_mode)
    {
        case LPC_LP_SLEEP:
            ...
            SYSTEM.SBYCR.BIT.SSBY   = 0;        // Move to Sleep on wait()
        ...
        case LPC_LP_ALL_MODULE_STOP:
            R_BSP_RegisterProtectDisable(BSP_REG_PROTECT_LPC_CGC_SWR);// unlock clock control registers
            SYSTEM.SBYCR.BIT.SSBY      = 0;     // Move to All Module Stop on wait(). ACSE also needs to be set
       ...
       break;
        ...
        case LPC_LP_SW_STANDBY:
            R_BSP_RegisterProtectDisable(BSP_REG_PROTECT_LPC_CGC_SWR);
            SYSTEM.SBYCR.BIT.SSBY    = 1;       // Move to Sw Stby on wait(); DPSBY also has to be modified
            SYSTEM.DPSBYCR.BIT.DPSBY = 0;
            SYSTEM.FHSSBYCR.BYTE = LPC_CFG_SW_STANDBY_OPTIONS; //Configure Sw Standby mode options
            R_BSP_RegisterProtectEnable(BSP_REG_PROTECT_LPC_CGC_SWR);
        break;
...
static lpc_err_t lpc_freq_range_check (lpc_operating_mode_t e_mode)
{
    ...
    clock_frequency[5] = 125000;           // IWDT - 125 kHz

    hoco_frequency[0] = 32000000; // 32 MHz
    hoco_frequency[1] = 36864000; // 36.864 MHz
    hoco_frequency[2] = 40000000; // 40 MHz
    hoco_frequency[3] = 50000000; // 50 MHz
    ...
    switch (e_mode)
    {
        ...
        break;
        case LPC_OP_MIDDLE_SPEED_1A:
            if (((((iclk_freq > LPC_MAX_ICK_MID1A_SPEED_FREQ_HZ)
                || (bclk_freq > LPC_MAX_BCK_MID1A_SPEED_FREQ_HZ))
                || (fclk_freq > LPC_MAX_FCK_MID1A_SPEED_FREQ_HZ))
                || (pclkd_freq > LPC_MAX_PCKD_MID1A_SPEED_FREQ_HZ))
                || (pclkb_freq > LPC_MAX_PCKB_MID1A_SPEED_FREQ_HZ))
            {
                return LPC_ERR_CLOCK_EXCEEDED;
            }
        break;
        case LPC_OP_MIDDLE_SPEED_1B:
            if (((((iclk_freq > LPC_MAX_ICK_MID1B_SPEED_FREQ_HZ)
                || (bclk_freq > LPC_MAX_BCK_MID1B_SPEED_FREQ_HZ))
                || (fclk_freq > LPC_MAX_FCK_MID1B_SPEED_FREQ_HZ))
                || (pclkd_freq > LPC_MAX_PCKD_MID1B_SPEED_FREQ_HZ))
                || (pclkb_freq > LPC_MAX_PCKB_MID1B_SPEED_FREQ_HZ))
            {
                return LPC_ERR_CLOCK_EXCEEDED;
            }
        break;
        ...
        case LPC_OP_LOW_SPEED_2:
            if (((((iclk_freq > LPC_MAX_ICK_LOW2_SPEED_FREQ_HZ)
                || (bclk_freq > LPC_MAX_BCK_LOW2_SPEED_FREQ_HZ))
                || (fclk_freq > LPC_MAX_FCK_LOW2_SPEED_FREQ_HZ))
                || (pclkd_freq > LPC_MAX_PCKD_LOW2_SPEED_FREQ_HZ))
                || (pclkb_freq > LPC_MAX_PCKB_LOW2_SPEED_FREQ_HZ))
            {
                return LPC_ERR_CLOCK_EXCEEDED;
            }
...
static uint32_t lpc_clockhzget (lpc_system_clocks_t clock)
{
    ...
    if (LPC_CLOCK_ACTIVE == SYSTEM.PLLCR2.BIT.PLLEN)
    {
        ...
        pll_multiplier = (float) SYSTEM.PLLCR.BIT.STC + 1.0f;
...
static lpc_err_t lpc_lowpower_activate_check (void)
{
    ...
    if (0x1 == SYSTEM.SBYCR.BIT.SSBY) /* check if entering software standby or deep software standby */
    {
        ...
        if ((LPC_SUB_CLOCK == SYSTEM.SCKCR3.WORD)
           && ((1 != RTC.RCR3.BIT.RTCEN) && (LPC_CLOCK_ACTIVE != SYSTEM.LOCOCR.BYTE)))
        {
            return LPC_ERR_ILL_CLOCK_SOURCE; /* Cannot enter SW_Stanbay if RTC disable or LOCO stoped when subclock selected */
        }
            ...
            (LPC_CLOCK_SWITCH_ENABLE == SYSTEM.RSTCKCR.BIT.RSTCKEN)) /* If Return Clock Switching is enabled */
    {
        ...
        {
            if (((((0x00 == SYSTEM.SCKCR.BIT.ICK)
                || (0x00 == SYSTEM.SCKCR.BIT.BCK))
                || (0x00 == SYSTEM.SCKCR.BIT.FCK))
                || (0x00 == SYSTEM.SCKCR.BIT.PCKB))
                || (0x00 == SYSTEM.SCKCR.BIT.PCKD))
            {
                return LPC_ERR_ILL_CLOCK_SOURCE;    /* Cannot enter Sleep if clock diveided by 2 or more */
            }
