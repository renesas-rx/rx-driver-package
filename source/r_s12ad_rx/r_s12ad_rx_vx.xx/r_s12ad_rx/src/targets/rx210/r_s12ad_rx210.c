...
*           01.10.2016 2.20    Bug fix for parameter range check.
*                              Define name commoditize for the same meaning but different name define.
*                              Delete parameter check by the enum value.
*                              Delete unnecessary register setting.(TSCR.PGAEN=0 rewrite)
*                              Delete sample & hold circuit error check.(undocumented)
*                              Error check added for self-diag.
...
*
* Arguments    : unit -
*                    Ignored for rx210. Pass 0 for this argument.
*                mode-
                   ...
                   void         (* const  p_callback)(void *p_args))
{
    ...
    if ((mode == ADC_MODE_SS_TEMPERATURE) && (p_cfg->add_cnt != ADC_ADD_OFF))
    {
        return ADC_ERR_ILLEGAL_ARG;
    }
    ...
    if ((mode == ADC_MODE_SS_MULTI_CH_GROUPED) || (mode == ADC_MODE_SS_MULTI_CH_GROUPED_DBLTRIG_A))
    {
        if ((p_cfg->trigger == ADC_TRIG_ASYNC_ADTRG)
         ...
         || (p_cfg->trigger_groupb == ADC_TRIG_SYNC_TEMPS)
    ...
    if (mode == ADC_MODE_SS_TEMPERATURE)
    {
        S12AD.ADSTRGR.BIT.TRSA = ADC_TRIG_SYNC_TEMPS;   // synchronous temperature trigger
    }
...
adc_dda_t   *charge;
    ...
    case ADC_CMD_SET_DDA_STATE_CNT:
        ...
        {
            if ((charge->num_states == 0) || (charge->num_states > ADC_DDA_STATE_CNT_MAX))
            {
                return ADC_ERR_INVALID_ARG;
            }
        }
...
uint32_t    tmp_mask=0;
    ...
    /* Don't work self-diag with single scan mode(double trigger) or group scan mode(double trigger) */
    if (p_config->diag_method != ADC_DIAG_OFF)
    {
        if ((g_dcb.mode == ADC_MODE_SS_ONE_CH_DBLTRIG)
         || (g_dcb.mode == ADC_MODE_SS_MULTI_CH_GROUPED_DBLTRIG_A))
        {
            return ADC_ERR_ILLEGAL_ARG;
        }
    }
